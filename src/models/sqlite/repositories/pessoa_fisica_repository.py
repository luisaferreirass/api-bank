...
class PessoaFisicaRepository(PessoaFisicaRepositoryInterface):
        ...
        with self.__db_connection as database:
            try:
                ...
                if (database.session
                    ...
                    .filter(PessoaFisicaTable.nome_completo == nome_completo)
                    .count() > 1):
                    raise ValueError("Existem múltiplas pessoas com o mesmo nome")
                
                if not person:
                    raise ValueError("Nenhuma pessoa encontrada com esse nome")
    ...
    def get_user(self, nome_completo: str):
        with self.__db_connection as database:
            try:
                person = (database.session
                          .query(PessoaFisicaTable)
